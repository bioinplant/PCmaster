...
the_method = 'pcma_mygo_fc'
# if config_dict['annotation_mode'] == 'default':
#     the_method = 'pcma_mygo_fc'
# elif config_dict['annotation_mode'] == 'st':
#     the_method = 'pcma_mygo_rc'

if config_dict['identification_mode'] == 'default':
    unknown_mode = 'no_unknown'
elif config_dict['identification_mode'] == 'csi':
    unknown_mode = 'with_unknown'

if config_dict['run_mode'] == 'cpu_only':
    the_method = 'pcma_mygo_hp'

if config_dict['framework_mode'] == 'rc_weighted':
    the_method = 'pcma_mygo_rc'
...
def pcma_annotation_mygo(config_dict = None):
    save_dir = config_dict['save_dir']

    if not os.path.exists(save_dir):  
        os.makedirs(save_dir)
    result_records_finished = os.listdir(save_dir)
    # print(result_records_finished)

    obj_finished = set()
    for i in result_records_finished:
        if i.startswith('pre') and len(i.split(the_method+'_')) > 1:
            obj_finished.add(i.split(the_method+'_')[1].strip()[:-4])
        if i.startswith('re') and len(i.split(the_method+'_')) > 1:
            obj_finished.add(i.split(the_method+'_')[1].strip()[:-4])
    ...
    celltype_label = config_dict['celltype_label']

    if adata_mode == 'adata1-adata2':
        split_ratio=[0.8,0.2]
    elif adata_mode == 'adata':
        split_ratio=[0.8,0.1,0.1]

    time_sleep = 1
    celltype_filterlist = config_dict['celltype_filterlist']

    if the_method == 'cell-blast':
        if_norm=True
        if_hvg=True
        n_top_hvgs=2000 # in the original article, recommanded num of hvgs is between 500–5000
        if_df = False
        if_feature_len = False
        if_balance_train_data = False
        random_sample_num = 1500
        epochs=None # default is unlimited, when > 70, change is very small, in past benchmark, epochs=60
    elif the_method == 'scvi-tools':
        if_norm=True
        if_hvg=True
        n_top_hvgs=2000
        ...
        if_balance_train_data = True
        random_sample_num = 1500
    elif the_method == 'pcma_mygo_rc':
        if_norm=True
        if_hvg=False
        n_top_hvgs=None
        if_df = True
        if_feature_len = False # attention!!!
        if_balance_train_data = True
        random_sample_num = 1500
        the_batch_size=16
        lr=0.01
        ...
        dropout_ratio2=None
        dropout_ratio3=None
        epochs=6 # 6 recommanded
        gpu_code=0  # attention!!!
        the_num_workers=0
        the_optimizer='SGD'
        the_momentum=0.9
        the_length=224 # max feature_len is 224*224
    elif the_method == 'pcma_mygo_rc2':
        if_norm=True
        if_hvg=False
        n_top_hvgs=None
        if_df = True
        ...
        gpu_code=0
        the_num_workers=0
        the_num_heads=64 # for pcma_mygo_sa
        the_optimizer='Adam'
        the_momentum=None

    gpu_code = int(config_dict['gpu_code'])

    def model_train_and_predict(the_method):
        if the_method == 'cell-blast': 
            print(the_method)
            print(adata_mode)
            print(unknown_mode)
            model, test_label_unmapped,predicted_unmapped = train_Cell_BLAST_with_unknown(
                new_anndata=new_anndata_train,new_anndata2=new_anndata_test, # new_anndata = new_anndata_train; new_anndata2 = new_anndata_test
                epochs=epochs,
                mapping_1=None,mapping_2=None,mapping_1_test=None,mapping_2_test=None,
            )
        elif the_method == 'scvi-tools':
            ...
            print(adata_mode)
            print(unknown_mode)
            model, test_label_unmapped,predicted_unmapped = train_scvi_tools_no_unknown(
                new_anndata_train=new_anndata_train,new_anndata_test=new_anndata_test, # new_anndata = new_anndata_train; new_anndata2 = new_anndata_test
                epochs=epochs,
                mapping_1=None,mapping_2=None,mapping_1_test=None,mapping_2_test=None,
            )
        elif the_method == 'pcma_mygo_fc':
            if unknown_mode == 'no_unknown':
                print(the_method)
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_FC_no_unknown(
                    ...
                    mapping_1=mapping_1,mapping_2=mapping_2,mapping_1_test=mapping_1_test,mapping_2_test=mapping_2_test,
                )
            elif unknown_mode == 'with_unknown':
                print(the_method)
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_FC_with_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    the_batch_size=the_batch_size,num_classes=num_classes,
            ...
            if unknown_mode == 'no_unknown':
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_HP_no_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    mapping_1=mapping_1,mapping_2=mapping_2,mapping_1_test=mapping_1_test,mapping_2_test=mapping_2_test,
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_HP_with_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    mapping_1=mapping_1,mapping_2=mapping_2,mapping_1_test=mapping_1_test,mapping_2_test=mapping_2_test,
            ...
            if unknown_mode == 'no_unknown':
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_RC_no_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    the_batch_size=the_batch_size,num_classes=num_classes,
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_RC_with_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    the_batch_size=the_batch_size,num_classes=num_classes,
            ...
            if unknown_mode == 'no_unknown':
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_RC2_no_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    the_batch_size=the_batch_size,num_classes=num_classes,
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_RC2_with_unknown(
                    train_data=train_data,train_label=train_label,
                    valid_data=valid_data,valid_label=valid_label,
                    test_data=test_data,test_label=test_label,
                    the_batch_size=the_batch_size,num_classes=num_classes,
            ...
            if unknown_mode == 'no_unknown':
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_SA_no_unknown(
                train_data=train_data,train_label=train_label,
                valid_data=valid_data,valid_label=valid_label,
                test_data=test_data,test_label=test_label,
                the_batch_size=the_batch_size,num_classes=num_classes1,
                lr=lr,dropout_ratio1=dropout_ratio1,dropout_ratio2=dropout_ratio2,dropout_ratio3=dropout_ratio3,feature_len=feature_len,
                ...
                print(adata_mode)
                print(unknown_mode)
                model, test_label_unmapped,predicted_unmapped = train_PCMA_MYGO_SA_with_unknown(
                train_data=train_data,train_label=train_label,
                valid_data=valid_data,valid_label=valid_label,
                test_data=test_data,test_label=test_label,
                the_batch_size=the_batch_size,num_classes=num_classes1,
    ...
    for random_seed in random_seeds:
        ...
        for obj in objs:
            gc.collect()
            # try:
            if not obj.endswith('.h5ad'):
                continue
            if obj.replace('.h5ad','')+'_'+str(random_seed) in obj_finished:
                print(obj.replace('.h5ad','')+'_'+str(random_seed))
                print('passed')
                continue
            else:
                print(obj.replace('.h5ad','')+'_'+str(random_seed))
            ...
            if obj_list is not None:
                if obj not in obj_list:
                    print(f'{obj} not in obj_list, passed')
                    continue
            ...
            result_record = ''
            result_record = result_record+'obj: '+str(obj.replace('.h5ad',''))+' []'
            result_record = result_record+'random_seed: '+str(random_seed)+' []'
            
            file_path1 = root_dir+obj.split('---')[0]
            if adata_mode=='adata1-adata2':
                file_path2 = root_dir+obj.split('---')[1]
            new_anndata1 = sc.read(file_path1)
            print(new_anndata1)
            if adata_mode=='adata1-adata2':
                new_anndata2 = sc.read(file_path2)
                print(new_anndata2)
            new_anndata1.obs['cellname'] = new_anndata1.obs_names
            new_anndata1.obs['celltype'] = new_anndata1.obs[celltype_label]
            if adata_mode=='adata1-adata2':
                new_anndata2.obs['cellname'] = new_anndata2.obs_names
                new_anndata2.obs['celltype'] = new_anndata2.obs[celltype_label]
            for filter_celltype in celltype_filterlist:
                new_anndata1 = new_anndata1[new_anndata1.obs['celltype']!=filter_celltype,:]
            if adata_mode=='adata1-adata2':
                for filter_celltype in celltype_filterlist:
                    new_anndata2 = new_anndata2[new_anndata2.obs['celltype']!=filter_celltype,:]

            print('new_anndata1 before filter new_anndata2')
            print(new_anndata1)
            # new_anndata1 = new_anndata1[new_anndata1.obs['Dataset']!=new_anndata2.obs['Dataset'][0],:]
            print('new_anndata1 after')
            print(new_anndata1)
                    
            ref_list = []
            for i in new_anndata1.obs[celltype_label]:
                ref_list.append(i)
            _,ref_mapping,_ = generate_mapping_for_list(input_list = ref_list)
            
            numofcells1 = len(new_anndata1.obs_names)
            if adata_mode=='adata1-adata2':
                numofcells2 = len(new_anndata2.obs_names)
            result_record = result_record+'numofcells1: '+str(numofcells1)+' []'
            if adata_mode=='adata1-adata2':
                result_record = result_record+'numofcells2: '+str(numofcells2)+' []'
            
            if adata_mode=='adata1-adata2':
                new_anndata = preprocessing(input_adata=[new_anndata1,new_anndata2],if_filter_genes=True,
                                if_norm=if_norm,if_hvg=if_hvg,n_top_hvgs=n_top_hvgs)
                if the_method=='pcma_mygo_rc':
                    if len(new_anndata.var_names)>50000:
                        sc.pp.highly_variable_genes(new_anndata,n_top_genes=50000)
                        sc.pl.highly_variable_genes(new_anndata)
                        new_anndata = new_anndata[:, new_anndata.var.highly_variable]
                elif the_method=='pcma_mygo_sa':
                    if len(new_anndata.var_names)!=2048:
                        new_anndata = new_anndata[:, :2048]
                new_anndata1 = new_anndata[new_anndata.obs['ref_test']=='ref',:]
                new_anndata2 = new_anndata[new_anndata.obs['ref_test']=='test',:]
                del new_anndata
                gc.collect()
            elif adata_mode=='adata':
                new_anndata1 = preprocessing(input_adata=new_anndata1,if_filter_genes=True,
                                if_norm=if_norm,if_hvg=if_hvg,n_top_hvgs=n_top_hvgs)
                if the_method=='pcma_mygo_rc':
                    if len(new_anndata1.var_names)>50000:
                        sc.pp.highly_variable_genes(new_anndata1,n_top_genes=50000)
                        sc.pl.highly_variable_genes(new_anndata1)
                        new_anndata1 = new_anndata1[:, new_anndata1.var.highly_variable]
                elif the_method=='pcma_mygo_sa':
                    if len(new_anndata1.var_names)!=2048:
                        new_anndata1 = new_anndata1[:, :2048]
            
            print(file_path1)
            print_anndata(input_adata=new_anndata1)
            if adata_mode=='adata1-adata2':
                print(file_path2)
                print_anndata(input_adata=new_anndata2)
            
            num_classes1 = len(new_anndata1.obs[celltype_label].value_counts())
            if adata_mode=='adata1-adata2':
                num_classes2 = len(new_anndata2.obs[celltype_label].value_counts())
            
            numofcelltypes1 = num_classes1
            if adata_mode=='adata1-adata2':
                numofcelltypes2 = num_classes2
            result_record = result_record+'numofcelltypes1: '+str(numofcelltypes1)+' []'
            if adata_mode=='adata1-adata2':
                result_record = result_record+'numofcelltypes2: '+str(numofcelltypes2)+' []'
            
            feature_len = len(new_anndata1.var_names)
            print(f'feature_len = {feature_len}')
            
            print_memory_used()
            
            if adata_mode=='adata1-adata2':
                new_anndata_train, new_anndata_valid = split_anndata_by_celltype_to_train_valid_test(
                    new_anndata=new_anndata1, celltype_label=celltype_label, split_ratio=split_ratio, random_seed=random_seed,
                )
                new_anndata_test = new_anndata2
            elif adata_mode=='adata':
                new_anndata_train, new_anndata_valid, new_anndata_test = split_anndata_by_celltype_to_train_valid_test(
                    new_anndata=new_anndata1, celltype_label=celltype_label, split_ratio=split_ratio, random_seed=random_seed,
                )
                
            test_data_cell_names = new_anndata_test.obs_names
            
            if adata_mode=='adata1-adata2':
                del new_anndata1,new_anndata2
            elif adata_mode=='adata':
                del new_anndata1
            gc.collect()
            
            if the_method == 'pcma_mygo_rc':
                the_length = the_length # 224
            else:
                the_length = int(math.sqrt(feature_len) + 1)
                
            if if_df is True:
                train_data,train_label,valid_data,valid_label,test_data,test_label,mapping_1,mapping_2,category_counts,mapping_1_test,mapping_2_test = \
                generate_datas_and_labels(
                    ...
                    random_sample_num = random_sample_num,if_need_test_mapping=True,
                )
            ...
            model,test_label_unmapped,predicted_unmapped = model_train_and_predict(the_method=the_method)
            
            print(file_path1)
            if adata_mode=='adata1-adata2':
                print(file_path2)
            print('test_label_unmapped')
            print_list(input_list=test_label_unmapped)
            print('predicted_unmapped')
            print_list(input_list=predicted_unmapped)
            result_records.append(result_record)
            # with open(save_dir+'result_record_'+the_method+'_'+obj.replace('-filtered-to-be-common.h5ad','')+'_'+str(random_seed)+'.pkl','wb') as file:
            #     pickle.dump(result_record,file)
            with open(save_dir+'test_label_'+str(obj)+'_'+str(random_seed)+'.pkl','wb') as file:
                pickle.dump(test_label_unmapped,file)
            with open(save_dir+'predicted_'+str(obj)+'_'+str(random_seed)+'.pkl','wb') as file:
                pickle.dump(predicted_unmapped,file)
            with open(save_dir+'ref_mapping_'+str(obj)+'_'+str(random_seed)+'.pkl','wb') as file:
                pickle.dump(ref_mapping,file)
            with open(save_dir+'test_data_cell_names_'+str(obj)+'_'+str(random_seed)+'.pkl','wb') as file:
                pickle.dump(test_data_cell_names,file)
            end_time = time.time()
