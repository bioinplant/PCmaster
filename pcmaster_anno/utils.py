...
def save_predictions(df: pd.DataFrame, out_path: str):
    """Save predictions DataFrame and return accuracy."""
    df.to_csv(out_path, index=False)
    acc = (df["pred_label"] == df["orig_celltype"]).mean()
    print(f"Saved predictions to {out_path}, acc={acc:.4f}")
    return acc
