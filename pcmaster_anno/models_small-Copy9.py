...
# ======================
# Improved 1D Residual CNN for RC_1D
# ======================

class ResidualBlock1D(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=7, stride=1, dropout=0.16):
        super().__init__()
...
class PCMA_MYGO_RC_1D(nn.Module):
    def __init__(self, num_classes=2, input_len=16384, base_channels=32, dropout=0.26, dropout_fc=0.18):
        super().__init__()
        ...
        # Progressive blocks (like ResNet)
        # self.layer1 = ResidualBlock1D(base_channels, base_channels, stride=1, dropout=dropout)
        self.layer2 = ResidualBlock1D(base_channels, base_channels * 2, stride=2, dropout=dropout)
        self.layer3 = ResidualBlock1D(base_channels * 2, base_channels * 4, stride=2, dropout=dropout)
        # self.layer4 = ResidualBlock1D(base_channels * 4, base_channels * 4, stride=1, dropout=dropout)

        self.pool = nn.AdaptiveAvgPool1d(1)
        self.fc1 = nn.Linear(base_channels * 4, base_channels)
        self.drop_fc = nn.Dropout(dropout_fc)
        self.fc2 = nn.Linear(base_channels, num_classes)
