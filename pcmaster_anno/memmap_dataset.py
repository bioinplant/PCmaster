...
class MemmapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ("fc", "sa", "hp"):
            ...
            # keep 1D flat vector, optionally pad to fixed length
            target_len = getattr(self, "target_len", None)  # can be set externally
            arr = row.astype("float32")
            if target_len is not None:
                if len(arr) < target_len:
                    ...
                    arr = arr[:target_len]
            return torch.from_numpy(arr), label, cellname
        else:
