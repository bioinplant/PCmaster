...
class PCmaster_anno_0():
    ...
    def annotation_with_marker_gene_list_0(self,marker_select_flag = None,marker_grade_1_coefficient = None):
        # 不变默认情况的更新，这是marker#1的权重，只在marker#1和marker#2混用时，且一个基因是marker#1基因且只对应一个细胞类型时起作用
    ...
    def to_annotation_0(self, verbosity=None, dpi=None, facecolor=None, # set_default_0
            ...
            pca_svd_algorithm=None, pca_genelist=None, # pca_cluster_markerSelect_0
                ...
                markerSelect_method=None,  # 'wilcoxon','logreg' # is ok too
                markerSelect_n_genes=None, show_n=None,
            marker_select_flag = None,marker_grade_1_coefficient = None):
        if filepath is not None:
        ...
        if self.obj is not None:
            ...
            self.annotation_with_marker_gene_list_0(marker_select_flag = marker_select_flag,marker_grade_1_coefficient = marker_grade_1_coefficient)
    ...
    def auto_anno_mldl_train_0(self,ref_obj=None,test_obj=None,gpu_code = None):
        ...
        sc.pp.normalize_total(ref_obj, target_sum=1e4)
        sc.pp.normalize_total(test_obj, target_sum=1e4)
        ref_obj.obs['cellname'] = ref_obj.obs_names
        test_obj.obs['cellname'] = test_obj.obs_names
        ...
        label=pd.concat([ref_obj.obs['cellname'], ref_obj.obs['celltype']], axis=1)
        df = label
        # 获取"celltype"中值的类别数
        ...
        train_iter = DataLoader(traindata,batch_size=16,shuffle=False,sampler=train_sampler,num_workers=4)
        valid_sampler = RandomSampler(validdata)
        valid_iter = DataLoader(validdata,batch_size=16,shuffle=False,sampler=valid_sampler,num_workers=4)
